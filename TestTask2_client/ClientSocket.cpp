...
void ClientSocket::ConnectToServer(string ip_addr, int port)
{
	try 
	{
		...
		if (Res < 0)
		{
			throw Exception(13, "ClientSocket: can not connect to server - address family is not valid.");
		}
		else if (Res == 0)
		{
			throw Exception(14, "ClientSocket: can not connect to server - ip address is not valid.");
		}
...
int ClientSocket::SendFile(string filename)
{
	...
	//send filename
	vector<char> filename_buf(filename.begin(), filename.end());
...
void ClientSocket::ReadFile(string filename, vector<char>& filedata_buf)
{
	size_t length = 0;
	ifstream ifs;
...
void ClientSocket::ExtractFilename(string &filename)
{
